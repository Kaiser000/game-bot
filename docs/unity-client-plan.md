...
- 视角：暂不定死。第一轮 Unity 原型做两版效果对比：鹅鸭杀式斜俯视、第三人称进房间；看实际手感、表现力和制作成本后再拍板。
- 美术：哥特童话 + 动物拟人，保留悬疑感，但避免写实恐怖路线。
- 第一版局型：只做 9 人狼人杀预女猎，不同时扩阿瓦隆和 12 人大局。
- AI 呈现：房主和设置知道 AI 数量，局内弱化 AI 标签，让 AI 更像桌上的玩家。
- 发行节奏：先做免费 Demo，用来验证体验、录制素材和积累愿望单；正式版再考虑买断。
- 语音策略：实时语音和 ASR 转写并行设计。真人之间听实时语音；AI、复盘和推理记忆使用语音转写后的文字与结构化摘要。AI 第一阶段文字输出，后续再加 TTS。
- 场景范围：第一版只做圆桌房间和入座流程，不做大地图和自由探索。
...
- 真人交流链路：实时语音，让玩家直接听到彼此发言。
- AI 理解链路：同一段语音进入 ASR，生成文字记录，再提炼为结构化关键信息。
- 对局记忆：保存“谁跳了什么身份、谁怀疑谁、谁站边谁、投票意向、语气强弱”等摘要。
- 第一阶段：语音按钮 UI、发言状态、文字记录和摘要接口占位。
- 第二阶段：接入实时语音服务或 Steam 语音能力，同时接入 ASR 转写。
- 第三阶段：空间化语音、说话角色高亮、AI TTS 输出。
...
- 语音服务成本、延迟和隐私合规。
- ASR 转写错误导致 AI 误判，需要保留原文、摘要和纠错机制。
